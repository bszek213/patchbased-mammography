...
def main():
    type_patch_total = ['small','medium', 'large']
    multiplication_values = ['1','1-25','1-50','1-75','2','2-25']

    early_stopping = EarlyStopping(monitor='val_loss', patience=10, restore_best_weights=True)
    lr_scheduler = LearningRateScheduler(schedule)

    for type_patch, multiply in zip(type_patch_total,multiplication_values):
        X_train = np.load(os.path.join('data_patch', f'{type_patch}_patches_{str(multiply)}_train.npy'))
        x_val = np.load(os.path.join('data_patch', f'{type_patch}_patches_{str(multiply)}_valid.npy'))
        X_test = np.load(os.path.join('data_patch', f'{type_patch}_patches_{str(multiply)}_test.npy'))

        y_train = np.load(os.path.join('data_patch', f'{type_patch}_patches_train_{str(multiply)}_label.npy'))
        y_test = np.load(os.path.join('data_patch', f'{type_patch}_patches_test_{str(multiply)}_label.npy'))
        y_val = np.load(os.path.join('data_patch', f'{type_patch}_patches_valid_{str(multiply)}_label.npy'))
        ...
        patch_architecture_res.save_model(f'patch_ResNet152_{type_patch}_patches_{str(multiply)}.h5')
        ...
        with open(f'test_results_dense_{type_patch}_{str(multiply)}.txt', 'w') as file:
            file.write(f'Test Accuracy: {test_results[1]}\n')
            file.write(f'Test Loss: {test_results[0]}\n')
        patch_architecture_dense.save_model(f'patch_dense121_{type_patch}_patches_{str(multiply)}.h5')